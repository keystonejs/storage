...
AmazonClient.prototype.remove = function (filename, callback) {
	this._connection.removeFile(this._config.container, filename, function (err) {
...
AmazonClient.prototype._init = function (callback) {
	this._connection.createContainer(this._config.container, function (err) {
		if(err) err.message = 'AmazonClient: There was a problem with initialization ' + err.message;
		callback(err);
	});
};
