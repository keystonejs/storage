...
MongoClient.prototype.upload = function (localSrc, destSrc, callback) {
	...
	writeStream.on('error', function (err) {
		if(err) err.message = 'MongoClient: There was a problem with uploading. Details: ' + err.message;
		callback(err);
	});
};
...
MongoClient.prototype.download = function (destSrc, localSrc, callback) {
	...
	readStream.on('error', function (err) {
		if(err) err.message = 'MongoClient: There was a problem with downloading. Details: ' + err.message;
		callback(err);
	});
...
MongoClient.prototype.remove = function (destSrc, callback) {
	...
	this._connection.remove({
		...
		if(err) err.message = 'MongoClient: There was a problem with removing. Details: ' + err.message;
		callback(err);
	});
