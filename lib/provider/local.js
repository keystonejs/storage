...
LocalClient.prototype.__ensureContainer = function (callback) {
	fs.ensureDir(this.__config.container, callback);
};
...
LocalClient.prototype.upload = function (localSrc, destSrc, callback) {
	...
	waterfall([
		...
		function (callback) {
			fs.copy(localSrc, path.join(container, destSrc), function (err) {
				if (err) {
...
LocalClient.prototype.download = function (destSrc, localSrc, callback) {
	if (path.extname(destSrc)) {
		fs.copy(path.join(this.__config.container, destSrc), localSrc, function (err) {
			callback(err);
		});
	} else {
