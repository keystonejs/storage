...
LocalClient.prototype.__ensureContainer = function (callback) {
	mkdir(this.__config.container, function (err) {
		if (err && err.errno !== 47) {
			callback(err);
		} else {
			callback();
...
LocalClient.prototype.upload = function (localSrc, destSrc, callback) {
	...
	waterfall([
		...
		function (callback) {
			var error = !path.extname(destSrc) ? new Error('LocalSystem: Can\'t upload as a directory') : null;
			callback(error);
		},
		function (callback) {
...
LocalClient.prototype.remove = function (destSrc, callback) {
	if (path.extname(destSrc)) {
		unlink(path.join(this.__config.container, destSrc), function (err) {
			callback(err);
		});
	} else {
		callback(new Error('LocalSystem: No file to remove under given path - ' + destSrc));
	}
};
