...
DropboxClient.prototype.remove = function (fileName, callback) {
	...
	var fileSrc = path.join(this._config.folder, fileName),
		client = this._connection;

	client.remove(fileSrc, callback);

};

DropboxClient.prototype.download = function (fileName, fileDest, callback) {
	...
	async.waterfall([
		...
		function (callback) {
			client.readFile(fileSrc, function (err, data) {
				callback(err, data);
			});
		},
		...
		function (data, callback) {
			client.writeFile(fileDest, data, callback);
		}
